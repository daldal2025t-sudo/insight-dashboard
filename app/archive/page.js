...
export default function ArchivePage() {
  ...
  const [etfData, setEtfData] = useState({ aggressive: [], neutral: [], stable: [], all: [] });
  ...
  useEffect(() => {
    ...
    if (savedQuantities) {
      try { setQuantities(JSON.parse(savedQuantities)); } catch (e) {}
    }
  ...
  const computedAllItems = (etfData.all || []).map(item => {
  ...
  const finalAllItemsWithWeight = computedAllItems.map(item => {
    return { ...item, realWeight: totalPortfolioValue > 0 ? (item.evalValue / totalPortfolioValue) * 100 : 0 };
  });

  const getPercentage = (subValue) => totalPortfolioValue > 0 ? (subValue / totalPortfolioValue) * 100 : 0;

  const sectorsFinal = [
    { label: '정보기술 (Tech)', val: getPercentage(sectorTotals.tech) },
    { label: '금융서비스 (Finance)', val: getPercentage(sectorTotals.finance) },
    { label: '헬스케어 (Health)', val: getPercentage(sectorTotals.health) },
    { label: '산업재 (Industrials)', val: getPercentage(sectorTotals.ind) },
    { label: '순환소비재 (Cyclical)', val: getPercentage(sectorTotals.consumer_cyc) },
    { label: '통신미디어 (Telecom)', val: getPercentage(sectorTotals.communication) },
    { label: '필수소비재 (Defensive)', val: getPercentage(sectorTotals.consumer_def) },
    { label: '에너지 (Energy)', val: getPercentage(sectorTotals.energy) },
    { label: '유틸리티 (Utilities)', val: getPercentage(sectorTotals.utilities) },
    { label: '기초소재 (Materials)', val: getPercentage(sectorTotals.basic) },
        ...
        {data.map((item, idx) => (
          <div key={idx} className="flex flex-col gap-1">
            <div className="flex justify-between text-xs font-bold text-gray-700">
              ...
              <span className={purple ? 'text-purple-700' : 'text-sky-600'}>{item.val.toFixed(2)}%</span>
            </div>
            <div className="w-full bg-gray-100 h-3 rounded-full overflow-hidden">
              <div 
                className={`h-full transition-all duration-500 ${purple ? 'bg-purple-700' : 'bg-sky-500'}`} 
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans">
      <header className="max-w-4xl mx-auto mb-8 flex justify-between items-center border-b border-gray-200 pb-6">
        <div>
          <p className="text-blue-600 font-bold text-xs md:text-sm tracking-wider">PORTFOLIO ARCHIVE</p>
          <h1 className="text-xl md:text-3xl font-extrabold text-gray-900 mt-1">나만의 ETF 포트폴리오</h1>
        </div>
      ...
      <main className="max-w-4xl mx-auto">
        ...
        {activeTab !== 'checker' && (
          <section className="bg-white rounded-2xl p-4 md:p-6 shadow-sm border border-gray-100">
            <div className="flex justify-between text-[10px] md:text-xs font-bold text-gray-400 border-b border-gray-100 pb-3 mb-3 px-2">
              <span>ETF 종목명</span>
              <span>현재가 / 등락률</span>
            </div>
            {isLoading ? (
              <div className="text-center py-12 text-gray-400 font-bold text-sm">데이터 수집 중... ⏳</div>
            ) : (
                  ...
                  <li key={index} className="flex justify-between items-center p-3 md:p-4 hover:bg-gray-50 rounded-xl transition gap-4">
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <p className="font-bold text-gray-900 text-base md:text-lg truncate">{etf.name}</p>
                        <span className="bg-blue-100 text-blue-700 text-[9px] md:text-[10px] px-1.5 py-0.5 rounded-full font-bold">{etf.weight}</span>
                      </div>
          ...
          <section className="flex flex-col gap-6">
            ...
            <div className="bg-black text-white p-6 rounded-2xl shadow-md flex justify-between items-center">
              <div>
                ...
                <p className="text-2xl md:text-3xl font-black text-white tracking-tight mt-1">
                  {totalPortfolioValue.toLocaleString('ko-KR')}<span className="text-sm font-normal text-gray-400 ml-1">원</span>
                </p>
              </div>
              <span className="bg-white/10 border border-white/20 px-3 py-1 rounded-full font-bold text-xs text-gray-300">실시간 연동</span>
            </div>

            {/* 2. 보유 비중 체크 입력 폼 리스트 (부자님의 요청으로 최상단 전면 배치) */}
            <div className="bg-white rounded-2xl p-4 md:p-6 shadow-sm border border-gray-100">
              <div className="grid grid-cols-12 text-[10px] md:text-xs font-bold text-gray-400 border-b border-gray-100 pb-3 mb-3 px-2">
                ...
                <span className="col-span-3 md:col-span-2 text-center">보유개수</span>
                <span className="col-span-4 text-right">실시간 평가액 / 비중</span>
              </div>

              {isLoading ? (
                <div className="text-center py-12 text-gray-400 font-bold text-sm">계산 시스템 동기화 중... ⏳</div>
              ) : (
                    ...
                    <div key={index} className="grid grid-cols-12 items-center px-2 py-1 border-b border-gray-50 last:border-0 pb-3 last:pb-0 gap-2">
                      ...
                      <div className="col-span-3 md:col-span-2 flex justify-center">
                        <input 
                          ...
                          min="0"
                          placeholder="0"
                          value={etf.qty === 0 ? '' : etf.qty}
                          onChange={(e) => handleQtyChange(etf.code, e.target.value)}
                          className="w-full max-w-[70px] md:max-w-[100px] text-center border border-gray-300 rounded-lg p-1 text-sm font-bold bg-gray-50 focus:bg-white focus:ring-2 focus:ring-black outline-none transition"
                        />
                      </div>
                      <div className="col-span-4 text-right flex flex-col justify-center">
                        <span className="text-sm md:text-base font-extrabold text-gray-900 tracking-tight">
                          {etf.evalValue.toLocaleString('ko-KR')}<span className="text-[10px] md:text-xs font-normal text-gray-400 ml-0.5">원</span>
                        </span>
                        <div className="flex items-center justify-end gap-1.5 mt-1">
                          <span className="text-xs font-black text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded tracking-tighter">
            ...
            {/* 3. 모닝스타 스타일 X-Ray 시각화 레포트 영역 (보유 비중 체크 입력 리스트 아래로 이동) */}
            {totalPortfolioValue > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 transition-all duration-500">
                <div className="md:col-span-2">
                  <RenderBarChart title="📊 실시간 포트폴리오 섹터별 비중 분석 (Sector Weight)" data={sectorsFinal} purple={true} />
                </div>
