...
- `/journal` (`app/journal/page.js`) — two tabs: 매매일지 (trade journal: stock name (optional), ticker symbol (required), buy date, buy price, quantity, free-text rationale per entry; persisted to `localStorage` under `kijay_trade_journal`, so private to whichever browser/device created it) and 종목 진단 (10-item fundamentals checklist + a Benjamin Graham fair-value calculator, moved here from `/archive`; this tab's state — checkbox states, Graham calculator inputs — is not persisted, it resets on reload, matching its original behavior on `/archive`). Entries created before the stock-name/ticker split used a single combined `ticker` field (e.g. `"엔비디아(NVDA)"`); `migrateEntry()` in `app/journal/page.js` converts those on load by extracting a ticker from trailing parentheses, leaving `symbol` blank (and price/return showing `-`) when it can't. The journal fetches live current prices for every unique `symbol` in the list from `/api/journal-quotes` (no caching — real-time, refetched whenever the entry list changes or the user clicks "시세 새로고침") and shows the return vs. `buyPrice` per entry. It also has "📋 표로 복사" (copies the currently-filtered list as tab-separated text via `copyTextToClipboard()`, pasteable straight into Google Sheets/Excel — falls back to a hidden-textarea `execCommand('copy')` when `navigator.clipboard` is unavailable, e.g. non-HTTPS) and "다른 기기로 옮기기" (export encodes all entries as JSON → UTF-8 bytes → base64 via `encodeToCode()`; import reverses that and merges by `id`, skipping entries whose `id` already exists so re-importing is idempotent and never overwrites existing data).
